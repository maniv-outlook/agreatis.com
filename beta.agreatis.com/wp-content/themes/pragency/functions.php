<?php
/**
 * PRagency functions and definitions.
 */
add_filter( 'kava-theme/assets-depends/styles', 'pragency_styles_depends' );
add_action( 'jet-theme-core/register-config', 'pragency_core_config' );
add_action( 'init', 'pragency_plugins_wizard_config', 9 );
add_action( 'init', 'pragency_data_importer_config', 9 );
add_action( 'tgmpa_register', 'pragency_register_required_plugins' );
/**
 * Enqueue styles.
 */
function pragency_styles_depends( $deps ) {
 ...
 */
function pragency_core_config( $manager ) {
	$manager->register_config(
		array(
			'dashboard_page_name' => esc_html__( 'PRagency', 'pragency' ),
 ...
 * @return [type] [description]
 */
function pragency_plugins_wizard_config() {
	if ( ! is_admin() ) {
		return;
	}
		...
		'skins'   => array(
			...
			'advanced' => array(
				'default' => array(
					...
					'name'  => esc_html__( 'PRagency', 'pragency' ),
					),
				),
			),
		'texts'   => array(
			'theme-name' => esc_html__( 'PRagency', 'pragency' ),
		)
	) );
}
/**
 ...
 * @return [type] [description]
 */
function pragency_data_importer_config() {
	if ( ! is_admin() ) {
		return;
	}
	...
	jet_data_importer_register_config( array(
		...
		'advanced_import' => array(
			'default' => array(
				'label'    => esc_html__( 'PRagency', 'pragency' ),
				...
				'demo_url' => 'https://ld-wp73.template-help.com/wordpress/prod_28125/v1',
			),
		),
		'import' => array(
			'chunk_size' => 3,
...
*/
		...
		'slider' => array(
			'path' => 'https://raw.githubusercontent.com/ZemezLab/kava-pragency/master/slides.json',
		),
	) );
}
/**
 ...
 */
function pragency_register_required_plugins() {
	$plugins = array(
		array(
			'name'         => esc_html__( 'Jet Plugin Wizard', 'pragency' ),
